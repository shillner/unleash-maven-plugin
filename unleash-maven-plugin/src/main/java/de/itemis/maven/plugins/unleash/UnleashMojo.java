...
public class UnleashMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		for (MavenProject project : reactorProjects) {
			...
			try {
				//loads a plain model from the project file since project.getModel() would return a pom with mixed-in parent declarations and would thus not be suitable for writing back
				...
				//TODO maybe do not just serialize the model but adapt the versions by hand to preserve the exact formatting of the pom
		...
		if (releaseVersion != null) {
			...
			if (currentVersion.endsWith(VERSION_MODIFIER_SNAPSHOT)) {
				String newVersion = currentVersion.substring(0,
					currentVersion.length() - VERSION_MODIFIER_SNAPSHOT.length());
